...
st.set_page_config(page_title="Patient Intake",
...
with col1:
    patient_id     = st.text_input(
        "Patient ID (leave blank to auto-generate)")
with col2:
...
with col3:
    treatment_type = st.selectbox("Treatment Type", [
        ...
        "Intensive Outpatient", "Standard Outpatient",
        "Medication-Assisted"
    ])
...
with col3:
    race = st.selectbox("Race / Ethnicity", [
        ...
        "American Indian / Alaska Native","Other / Mixed"])
with col4:
...
with col1:
    primary_substance = st.selectbox("Primary Substance", [
        ...
        "Cocaine / Crack","Methamphetamine","Marijuana","Other"])
    heroin     = st.selectbox("Heroin Use", ["No","Yes"])
with col2:
    frequency  = st.selectbox("Use Frequency", [
        "No Use","1-3 Times/Month","1-2 Times/Week",
        "3+ Times/Week","Daily"])
    needle_use = st.selectbox("Needle / IV Use", ["No","Yes"])
with col3:
    num_substances = st.number_input(
        "Number of Substances", min_value=1, max_value=6, value=1)
...
with col2:
    ...
    psych_problem = st.selectbox(
        "Psychiatric / Mental Health Condition", ["No","Yes"])
with col3:
...
arrests = st.selectbox(
    "Recent Arrests (past 30 days)",
    ["None","Once","Two or More"])

# ── CALCULATE RISK ────────────────────────────────────────────
st.divider()
submit = st.button("🔍 Calculate Risk Score",
...
if submit:
    ...
    FREQ_MAP    = {"No Use":0,"1-3 Times/Month":1,
                   "1-2 Times/Week":2,"3+ Times/Week":3,"Daily":4}
    ...
    SERVICES_MAP= {"Detox":1,"Residential Short-Term":2,
                   "Residential Long-Term":3,
                   "Intensive Outpatient":4,
                   "Standard Outpatient":5,
                   "Medication-Assisted":6}
    PSOURCE_MAP = {"Self / Family":1,
                   "Substance Abuse Provider":2,
                   "Other Healthcare Provider":3,
                   "School":4,"Employer":5,
                   "Criminal Justice":7,"Other":8}
    ...
    # Display risk score
    st.divider()
    bg = {"High":"#ffe5e5","Moderate":"#fff8e1",
          "Low":"#e8f5e9"}[risk_level]
    ic = {"High":"🔴","Moderate":"🟡","Low":"🟢"}[risk_level]

    st.markdown(f"""
    <div style="background:{bg};padding:24px;
                border-radius:12px;text-align:center">
        <h2>{ic} {risk_level} Risk of Dropout</h2>
        <h1 style="font-size:56px;margin:0">{risk_pct}%</h1>
        <p>Model confidence this patient will leave treatment early</p>
    </div>
    ...
    if st.button("💾 Save Patient Record",
        ...
        if success:
            st.success(
                f"✅ Patient {final_id} saved to Google Sheets.")
            st.info(
                f"Risk: {risk_pct}% — {risk_level} | "
                f"Saved: {new_record['timestamp']}")
        else:
            st.error("Save failed. Check Sheets credentials.")
